...
class State:
    ...
    def __init__(
        ...
        self.last_language = "en"  # Track last used language for force_transcription
    ...
    def _extract_transcriptions(
        ...
        for word in ts_result.words:
            ...
            if word.end < last_pause.end:
                final_starts_at = (
                    word.start if final_starts_at is None else final_starts_at
                )
            ...
            # consider everything else as interim
                ...
                interim_starts_at = (
                    word.start if interim_starts_at is None else interim_starts_at
                )
        ...
        if final.strip():
            ...
            if cut_mark_bytes > 0:
                log.debug(
                    f"Participant {self.participant_id}: cut mark set at {cut_mark_bytes} bytes"
                )
                final_start_timestamp = self.working_audio_starts_at + int(
                    final_starts_at * 1000
                )
                final_audio = None
                ...
                if return_audio:
                    ...
                    final_audio = (
                        utils.get_wav_header([final_raw_audio], final_audio_length)
                        + final_raw_audio
                    )
                results.append(
                    self.get_response_payload(
                        ...
                        final_audio,
                        True,
                        probability=last_pause.probability,
                    )
                )
                ...
                if interim.strip():
                    self.working_audio_starts_at = timeline_before_trim + int(
                        last_pause.end * 1000
                    )
            else:
                ...
                results.append(
                    self.get_response_payload(
                        (final + interim).strip(),
                        self.working_audio_starts_at
                        + int(ts_result.words[0].start * 1000),
                    )
                )
                return results
        if interim.strip() != "":
            results.append(
                self.get_response_payload(
                    interim.strip(),
                    self.working_audio_starts_at + int(interim_starts_at * 1000),
                )
            )
        return results
    ...
    async def force_transcription(
        ...
        if ts_result is not None and ts_result.text.strip():
            self.last_language = ts_result.language  # Use detected language
            results = []
            start_timestamp = (
            ...
            if return_audio:
                ...
                final_audio = (
                    utils.get_wav_header([self.working_audio], final_audio_length)
                    + self.working_audio
                )
            results.append(
                self.get_response_payload(
                    ...
                    final_audio,
                    True,
                    probability=utils.get_phrase_prob(
                        len(ts_result.words) - 1, ts_result.words
                    ),
                )
            )
        self.reset()
    ...
    async def process(
        self, chunk: Chunk, previous_tokens: list[int]
    ) -> List[utils.TranscriptionResponse] | None:
    ...
    async def add_to_store(self, chunk: Chunk, tmp_working_audio: bytes = b""):
        ...
        log.debug(
            f"## Participant {self.participant_id}: speech timestamps {speech_timestamps}"
        )
        log.debug(
            f"## Participant {self.participant_id}: last speech timestamp {self.last_speech_timestamp}"
        )
        ...
        if (
            speech_timestamps
            and speech_timestamps[-1]["end"] != self.last_speech_timestamp
        ):
            ...
            if (
                speech_timestamps
                and audio_length_seconds - speech_timestamps[-1]["end"] >= 1
            ):
                log.debug(
    ...
    async def do_transcription(
        self, audio: bytes, previous_tokens: list[int]
    ) -> utils.WhisperResult | None:
        self.is_transcribing = True
        start = time.perf_counter_ns()
        loop = asyncio.get_running_loop()
        log.debug(
            f"Participant {self.participant_id}: starting transcription of {len(audio)} bytes."
        )
        try:
            ts_result = await loop.run_in_executor(
                None, utils.transcribe, [audio], None, previous_tokens
            )
